        ...
        <form action="{{ route('roles.update', ['role' => $role]) }}" method="POST">
            ...
            <div>
                ...
                <input id="role_name" type="text" name="name" value="{{ old('name', $role->name) }}"
                    ...
                    class="block text-3xl bg-transparent border-0 border-b border-slate-200 focus:border-green-500 focus:ring-transparent w-full placeholder:text-slate-400 dark:border-slate-700 dark:text-slate-300 @error('name') is-invalid @enderror">
                @error('name')
                    <div id="toast-title"
                        ...
                        @foreach ($permissions as $permission)
                            <li class="py-2">
                                <div>
                                    ...
                                    @else
                                        <input id="{{ $permission }}"
                                            class="mr-3 cursor-pointer rounded-full border-0 bg-slate-300 dark:bg-slate-700"
